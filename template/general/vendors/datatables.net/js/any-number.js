...
_anyNumberSort = function(a, b, high) {
	...
	var reg = /[;":'a-zA-Zа-яА-Я\\=`ё/\*++!@#$%\^&_№?><]/g;    

	a = Number(parseFloat(String(a).replace(/\s+/g, '').replace(reg, '').replace(',', '.')).toFixed(2));
	a = a !== null ? a : high;
	b = Number(parseFloat(String(b).replace(/\s+/g, '').replace(reg, '').replace(',', '.')).toFixed(2));
	b = b !== null ? b : high;
